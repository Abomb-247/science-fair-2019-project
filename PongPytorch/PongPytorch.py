...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 28 09:57:06 2019
...
def mish(input): 
    return input - 2 * input / ( torch.exp(input) * ( torch.exp(input) + 2 ) + 2 )
...
class Mish(nn.Module):
    ...
    def forward(self, input):
        return mish(input) # simply apply already implemented SiLU
...
class Policy(nn.Module):
    ...
    def ram_to_tensor(self, R):
        if R is None:
            return torch.zeros(1, 128)
        return torch.from_numpy(R.astype(np.float32)).unsqueeze(0)

    def pre_process(self, x, prev_x, r):
        df= self.pixels_to_tensor(x) - self.pixels_to_tensor(prev_x)
        return torch.cat((df,self.ram_to_tensor(r)))
    ...
    for it in range(100000):
        ...
        for ep in range(10):
            ...
            for t in range(190000):
                ...
                d_obs = policy.pre_process(obs, prev_obs, ram)
                with torch.no_grad():
